...
class ModelMgr():
    ...
    def train(self):
        ...
        model = self.get_my_model()  ## [4] 과적합 문제 해결 방안을 위한 모델을 로드하는 부분입니다.
    ...
    def get_hyperparameter(self):
        ...
        #######################
        hyper = dict()
        hyper['batch_size'] = BATCH_SIZE  ## 배치 사이즈 설정하는 부분 입니다.
        hyper['epochs'] = EPOCH  ## 에폭을 설정하는 부분 입니다.

        hyper['learning_rate'] = LEARNING_RATE  ## 학습률을 설정하는 부분입니다.
        hyper['optimizer'] = OPTIMIZER  ## default: SGD
                            #keras.optimizers.Adam
        return hyper
    ...
    def get_my_model(self):
        ...
        기타 문의 : qorgh3466@gmail.com (수업조교)
        '''
        # 설계 의도: 기존 Dropout(0.3) 구조에 L2 규제를 추가하여
        # 가중치가 과도하게 커지는 것을 방지하고 일반화 성능을 극대화.
        ...
        model.add(keras.layers.Dense(128, activation='relu',
                                     kernel_regularizer=tf.keras.regularizers.l2(0.001)))
        model.add(keras.layers.Dropout(0.3))
        model.add(keras.layers.Dense(self.target_class, activation='softmax'))
        return model
